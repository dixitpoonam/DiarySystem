...
Statement st,stftp,stftp1, stmtsp1,stmtsp11,stcdever,stcdever1,stmtsp111,stmt1,stmt2,stmtsp, st1,st23,st24, st2, st3,st4,st5,st6, st9, st11, st01, st12,stamc,stamc1 ,stwar1,stwar2,stwar3,stwar4;
...
<%
	...
	try{
		...
		stftp = conn1.createStatement();
		stftp1 = conn1.createStatement();
		stcdever1=conn1.createStatement();
		stcdever = conn1.createStatement();
...
<%	}
	...
	<% 
		String cdever = "select * from db_gps.t_ftplastdump where unitid = '"+unitid+"'";
		ResultSet rstcdever = stcdever1.executeQuery(cdever);
		
		if(rstcdever.next())
		{
				
		  codeVersion = rstcdever.getString("codeVersion");
...
// ***********************************transmission interval calculation ************
		...
		if(rsFTP.next()){
			...
			UnitType=rsFTP.getString("UnitType");//UnitType,stmin,b,WMSN
...
String bg1="";
	...
	String cdever = "select * from db_gps.t_ftplastdump where unitid = '"+unitid+"'";
	ResultSet rstcdever = stcdever.executeQuery(cdever);
	...
	String sqlFTP="select * from db_gps.t_ftplastdump where UnitID='"+unitid+"'";
	ResultSet rsFTP=stftp1.executeQuery(sqlFTP);
	if(rsFTP.next()){
		WMSN=rsFTP.getString("WMSN");
		int TXInterval=0,StInterval=0;
		if(rsFTP.getString("TXInterval").equalsIgnoreCase("-")){
