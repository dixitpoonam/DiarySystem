...
Statement st,st1,st2,st3,st4,st5,st6,st7,st8, st9,st10,st11,st12,st13,st14,st15,stmtsp1,stmtsp11,stmtsp,stmtsp111,stQuickTest,st23, st01,stmt1,stmt2,stamc,stamc1 ,stwar1,stwar2,stwar3,stwar4,cnt1,cnt2,cnt3,cnt4,cnt5,cnt6,cnt7,cnt8;
...
try
	{
	 ...
	 sqlcnt2="select count(*) as cnt8  from db_gps.t_ticketmastertest  ";
	  	ResultSet rs157=st5.executeQuery(sqlcnt2);
	  	System.out.println(">>>sqlcnt2<<<"+sqlcnt2);
	  	 	 if(rs157.next())
	  	 {
		  	...
		  	ResultSet rs151=st6.executeQuery(sqlcnt1);
		  	...
		  	ResultSet rs152=st7.executeQuery(sqlcnt);
	    ...
	    for (int counter = 0; counter < numRecords; counter++) 
		{
			...
			sqlcnt5="select count(*) as cnt5  from db_gps.t_livetickettest WHERE STATUS = 'open' and vehid in (select vehiclecode from db_gps.t_onlinedata where transporter ='"+currentRecord.getOwnerName()+"') ";
		  	ResultSet rs155=st2.executeQuery(sqlcnt5);
		  	...
		  	sqlcnt6="select count(*) as cnt6  from db_gps.t_ticketmastertest WHERE  vehid in (select distinct(vehiclecode) from db_gps.t_onlinedatacopy where transporter ='"+currentRecord.getOwnerName()+"') and STATUS = 'AutoClosed' or STATUS = 'Closed'  ";
		  	ResultSet rs156=st3.executeQuery(sqlcnt6);
			 System.out.println(">>>sqlcnt6<<<"+sqlcnt6);
		  	...
		  	sqlcnt4="select count(tcategory) as cnt7  from db_gps.t_ticketmastertest where vehid in (select distinct(vehiclecode) from db_gps.t_onlinedatacopy where transporter ='"+currentRecord.getOwnerName()+"')  ";
		  	ResultSet rs158=st4.executeQuery(sqlcnt4);
...
<%		
    	...
    	try
    		{
    		 ...
    		 System.out.println(">>>INSIDE TRY<<<");
    		    ...
    		    sqlcnt2="select count(*) as cnt8  from db_gps.t_ticketmastertest  ";
			  	ResultSet rs157=st12.executeQuery(sqlcnt2);
			  	System.out.println(">>>sqlcnt2<<<"+sqlcnt2);
			  	 ...
			  	 if(rs157.next())
			  	 {
				  	...
				  	ResultSet rs151=st13.executeQuery(sqlcnt1);
				  	System.out.println(">>>sqlcnt2<<<"+sqlcnt1);
				  	...
				  	sqlcnt="select count(*) as cnt9  from db_gps.t_ticketmastertest where status ='closed' or  status ='AutoClosed'  ";
				  	ResultSet rs152=st14.executeQuery(sqlcnt);
				  	System.out.println(">>>sqlcnt2<<<"+sqlcnt);
    		    ...
    		    for (int counter = 0; counter < numRecords; counter++) 
    			{
    				...
    				String Trans="select distinct(OperationalStatus) from db_gps.t_transporter where TypeValue='"+currentRecord.getOwnerName()+"' and OperationalStatus='Active'";
    				ResultSet rsTrans=st8.executeQuery(Trans);
    				if(rsTrans.next())
    				{
    					k++;
    					OperationalStatus=rsTrans.getString("OperationalStatus");
    				
    					System.out.println(" Trans Status :- "+OperationalStatus);
    				
    				sqlcnt5="select count(*) as cnt5  from db_gps.t_ticketmastertest WHERE STATUS = 'open' and vehid in (select distinct(vehiclecode) from db_gps.t_onlinedatacopy where transporter ='"+currentRecord.getOwnerName()+"') ";
    			  	ResultSet rs155=st9.executeQuery(sqlcnt5);
    			  	 System.out.println(">>>sqlcnt5<<<"+sqlcnt5);
    			  	 if(rs155.next())
    			  	 {
    			  		open=rs155.getString("cnt5");
    			  	...
    			  	sqlcnt6="select count(*) as cnt6  from db_gps.t_ticketmastertest WHERE  vehid in (select distinct(vehiclecode) from db_gps.t_onlinedatacopy where transporter ='"+currentRecord.getOwnerName()+"') and STATUS = 'AutoClosed' or STATUS = 'Closed'  ";
    			  	ResultSet rs156=st10.executeQuery(sqlcnt6);
    				 System.out.println(">>>sqlcnt6<<<"+sqlcnt6);
    			  	 if(rs156.next())
    			  	 {
    			  		close=rs156.getString("cnt6");
    			  	...
    			  	sqlcnt4="select count(tcategory) as cnt7  from db_gps.t_ticketmastertest where vehid in (select distinct(vehiclecode) from db_gps.t_onlinedatacopy where transporter ='"+currentRecord.getOwnerName()+"')  ";
    			  	ResultSet rs158=st11.executeQuery(sqlcnt4);
    			  	System.out.println(">>>sqlcnt4<<<"+sqlcnt4);
    	...
    	%>	
    			<tr <%=currentRecord.isOwnerBlocked() ? "bgcolor='yellow'" :""%>>
    			<td><%=k%> </td>
    			<td><b><a href="javascript:toggleDetails(<%=k%>,true);" title="Click Here"><%=currentRecord.getOwnerName()%></a></b>
    				<br/>	
    			<div class="popup" id="popup<%=i%>">
    				<table border="0" >
    				<tr><td><a href='callhistory.jsp?trans=<%=currentRecord.getOwnerName()%>' onclick="toggleDetails(<%=i%>,false);">Call HIstory</a></td></tr>
