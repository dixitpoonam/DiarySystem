...
Statement st,st1,st2,st3,st4,stmt3;
...
try{
	...
	System.out.println(">>>sql<in sql<<"+sql1);
	if(rst.next())
	{
		String cont_details =  rst.getString("ContDets");
		probsnc=rst.getString("CDate");
...
<% 
						String sql2="select distinct(LName) as LName from db_CustomerComplaints.t_location order by LName asc" ;		
						ResultSet rst2=st4.executeQuery(sql2);
						System.out.println("Loc "+sql2);
%>
...
<%
							...
							System.out.println("Loc "+rst2.getString("LName"));
%>
							<option value='<%=rst2.getString("LName")%>' <% if(rst.getString("VehAttLoc").equals(rst2.getString("LName"))){ out.print("Selected");} %>><%=rst2.getString("LName")%></option>			
<% 
							}
%>
...
<div id="dvPassport" style="display: none">
       <input type="text" name="addpass" id="txtPassportNumber" />
			...
			<tr id="unitrow3" style="display:none">
				...
				<td>
<%
					String sql3="select * from t_callocation1 where Tno='"+rst.getString("Tno")+"' order by DofAllocation desc";
					ResultSet rst1=st2.executeQuery(sql3);
					techname="-";
					if(rst1.next())
					{
						techname=rst1.getString("Technicion");
					}	
					String sql4="select distinct(TechName) as TechName from t_techlist where Available='Yes' order by techname";
					ResultSet rst3=st3.executeQuery(sql4);
%>
