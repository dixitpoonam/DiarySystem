...
<%
					...
					System.out.println("Special Comment :- "+sqlsp1);
...
%>
	...
	//int i=1, j=0;
			...
			String Vehcode="",unitid="",group1="", vehregno="",vehno="";
			Double lat1,lon1;
			while(rst1.next())
			{    System.out.println("In the first while loop");
				...
				vehno=rst1.getString("VehicleRegNo");
		/*		String VehicleCode=Vehcode;
				if(VehicleCode!=null||VehicleCode!=""){
				String driver="SELECT  "+ 
				"  			tsj.DriverName, tsj.DriverCode, tsj.startdate "+
			    "             FROM db_gps.t_startedjourney tsj INNER JOIN " +
	 ...
	 sql8="select status,Empname,Instruction,UnitID,VehicleRegNumber,InstalledDate,InstalledPlace,InstalledBy from t_vehicledetails where vehiclecode='"+Vehcode+"'";
          System.out.println("sql8------>"+sql8);
	 ResultSet rstrem=st2.executeQuery(sql8);
	 if(rstrem.next())
	 {
	 	...
	 	unitid=rstrem.getString("UnitID");
		//	System.out.println("unitid-->"+unitid);
			vehregno=rstrem.getString("VehicleRegNumber");
			try{
			instdte=rstrem.getString("InstalledDate");
		//	System.out.println("InstalledDate---->"+instdte);
			InstalledPlace=rstrem.getString("InstalledPlace");
			instby=rstrem.getString("InstalledBy");//Added by veena for installed by
		//	System.out.println("InstalledPlace---->"+InstalledPlace);
			...
			{
					try{
						instdte1=rstrem.getString("InstalledDate");
						}
						catch(Exception X)
						{
							instdte1="-";
	...
	/* int spcnt1=0;
	 	String sqlsp="select count(*) as cnt from t_techspecialrep where VehRegNo='"+vehno+"' and cmmt_type='Vehicle' and Closed='No' ";
	...
	//-------------------------------------------------------------------------------------------------------------
		...
		ResultSet rsFTP=st5.executeQuery(sqlFTP);
		System.out.println("FTP Last Dump :- "+sqlFTP);
		if(rsFTP.next()){
			WMSN=rsFTP.getString("WMSN");
			int TXInterval=0,StInterval=0;
			if(rsFTP.getString("TXInterval").equalsIgnoreCase("-")){
		...
		else
		{
		 ...
		 String sqlsp="select * from t_techspecialrep where ((VehRegNo='"+vehno+"' and cmmt_type='Vehicle') or(UnitId='"+unitid+"' and cmmt_type='Unit')) and Closed='No' order by EntDateTime desc Limit 1";
		//String sqlsp="select * from t_techspecialrep where (VehRegNo='"+vehno+"' or UnitId='"+rst1.getString("UnitID")+"') and (cmmt_type='Vehicle' or `cmmt_type`='Unit') and Closed='No' order by `EntDateTime` desc Limit 1";
	//	System.out.println(sqlsp);
		ResultSet rssp=stmtsp.executeQuery(sqlsp);
		System.out.println("SPCCCCCCCCCCC :- "+sqlsp);	
		if(rssp.next())
		{
		...
		String sqlrepdis="select SpComment from t_techspecialrep where VehRegNo='"+vehno+"' and cmmt_type='Vehicle'  and Closed='No' order by EntDateTime desc limit 1";
		System.out.println("aaaaaaaaaaa^^^^^^^^^^^^^ "+sqlrepdis);
	...
	try
	{
		String sqltct="select * from db_gps.t_livetickettest where vehid ='"+Vehcode+"' ";
		ResultSet tct=stwar3.executeQuery(sqltct);
		System.out.println("Ticket Query :-"+sqltct);
		if(tct.next())
		{
	...
	<td width="100px"><a href="javascript:toggleDetails(<%=i%>,true);" title="Click Here"><b><%=vehno%></b>
	</a>
	<br/>	
	<div class="popup" id="popup<%=i%>">
	<table border="0" >
...
if(bg.equals("red"))
{
	...
	String sql9="select * from t_unitreplacement where VehRegNo='"+vehno+"' or VehCode='"+Vehcode+"' and InstType='Rem' order by EntDate desc";
	System.out.println("Unit Replacement 1:- "+sql9);
	...
	if(rs9.next())
	{
		...
		//out.println("unitid");
		String sql11="select * from t_unitreplacement where VehRegNo='"+vehno+"' and NewUnitID='"+unitid.trim()+"' ";
		System.out.println("Unit Replacement 2:- "+sql11);
	...
	{
		String sql11="select * from t_unitreplacement where VehRegNo='"+vehno+"' or VehCode='"+Vehcode+"' order by InstDate desc limit 1";
		System.out.println("Unit Replacement 3:- "+sql11);
		ResultSet rs11 = st11.executeQuery(sql11);
	...
	if(bg.equals("lightgreen"))
	{
		...
		String sql9="select * from t_unitreplacement where VehRegNo='"+vehno+"' or VehCode='"+Vehcode+"' and InstType='DeActive' order by EntDate desc";	
		ResultSet rs9=st9.executeQuery(sql9);
		System.out.println("Unit Replacement 4:- "+sql9);
		if(rs9.next())
		{
			unitid=rs9.getString("OldUnitId");
			...
			String sql11="select * from t_unitreplacement where VehRegNo='"+vehno+"' and NewUnitID='"+unitid.trim()+"' ";
			ResultSet rs11 = st11.executeQuery(sql11);
			System.out.println("Unit Replacement 5:- "+sql11);
			//out.println(sql11);
			if(rs11.next())
			{
				simno=rs11.getString("MobNo");
...
%>
	...
	<tr><td><a href="addcomplaint.jsp?vid=<%=Vehcode%>&tid=<%=tid %>&category=<%=tctegory %>" onclick="toggleDetails(<%=i%>,false);">Add Complaint</a></td></tr>
	...
	<td>
	   ...
	   <a href="#" onclick="reset(<%=Vehcode%>);">Reset Device</a></td></tr>
	
	
	
	<tr><td><a href="inspection.jsp?vid=<%=Vehcode%>&tid=<%=tid %>&category=<%=tctegory %>&tid=<%=tid %>&category=<%=tctegory %>&warranty=<%=warranty%>" onclick="toggleDetails(<%=i%>,false);">Inspection</a></td></tr>
	<!--  <tr><td><a href="addcontact.jsp?transporter=<%=transporter%>" onclick="toggleDetails(<%=i%>,false);">Add Transporter Contact</a></td></tr>
...
if(MANAGER.equalsIgnoreCase(userrole)||SUPERADMIN.equalsIgnoreCase(userrole))
{  %>
	<tr><td><a href="repunit1.jsp?vid=<%=Vehcode%>&tid=<%=tid %>&category=<%=tctegory %>&warranty=<%=warranty%>" onclick="toggleDetails(<%=i%>,false);">Unit Replace</a></td></tr>
	<tr>
...
<%
	...
	<tr><td><a href="javascript: toggleDetails(<%=i%>,false);" onclick="window.open ('rep_history.jsp?vid=<%=Vehcode%>','win1','width=800,height=600,location=0,menubar=0,scrollbars=1,status=0,toolbar=0,resizable=0')" > View History</a></td></tr>
	 <tr>
...
String remdate="", oldunitwith="", withtech="";
String sql10="select * from t_unitreplacement where VehRegNo='"+vehno+"'   order by EntDate desc limit 1";
 
 System.out.println("UNITTTTT Replace :- "+sql10);
...
<%	}
	...
	<%
		...
		ResultSet rscomp=st.executeQuery(sql);
		System.out.println("Complaint :- "+sql);	
		if(rscomp.next())
		{
...
String sqlsp1="select * from t_techspecialrep where ((VehRegNo='"+vehno+"' and `cmmt_type`='Vehicle') or(`UnitId`='"+unitid+"' and  `cmmt_type`='Unit')) and Closed='No' order by `EntDateTime` desc";
//String sqlsp1="select * from t_techspecialrep where (VehRegNo='"+vehno+"' or UnitId='"+rst1.getString("UnitID")+"') and (cmmt_type='Vehicle' or `cmmt_type`='Unit') and Closed='No' order by `EntDateTime` desc Limit 1";
//String sqlsp1="select count(*) as cnt from t_techspecialrep where UnitId = '"+unitid+"' and cmmt_type='Unit' and Closed='No' ";
	ResultSet rssp2=stmtsp111.executeQuery(sqlsp1);
	System.out.println("Spc Comm :-"+sqlsp1);	
	while(rssp2.next())
	{
...
%>
	...
	<td><%=spcmmnt %></td>
		...
		<%-- <td><%=InstalledBy %></td> --%>
				...
				<input type="hidden" id="vehno<%=j%>" name="vehno<%=j%>" value="<%=vehno%>" />
...
String bg1="";
	 ...
	 if(rstrem1.next())
	 {
		...
		String b="NA",stmin="NA";
		String sqlFTP="select * from db_gps.t_ftplastdump where UnitID='"+unitid+"'";
		System.out.println(" FTP Dump :-"+sqlFTP);
		ResultSet rsFTP=st5.executeQuery(sqlFTP);
		if(rsFTP.next()){
			WMSN=rsFTP.getString("WMSN");
			int TXInterval=0,StInterval=0;
	...
	try
	{
		String sqltct="select * from db_gps.t_livetickettest where vehid ='"+Vehcode+"' ";
		ResultSet tct=stwar3.executeQuery(sqltct);
		System.out.println("Ticket :- "+sqltct);
		if(tct.next())
		{
...
%>  
	...
	ResultSet rscomp1=st.executeQuery(sql);
	 System.out.println("COMP Laint "+sql);
	if(rscomp1.next())
	{
...
//String sqlsp1="select * from t_techspecialrep where (VehRegNo='"+vehno+"' or UnitId='"+rst1.getString("UnitID")+"') and (cmmt_type='Vehicle' or `cmmt_type`='Unit') and Closed='No' order by `EntDateTime` desc";
